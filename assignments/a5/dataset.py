...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        masked_len = int(truncated_len/4)
        var = min(masked_len-1, truncated_len-2)
        masked_len = random.randint(masked_len-var, masked_len+var)
        cut_pos = random.randint(1, truncated_len-masked_len-1)
        prefix = truncated_doc[:cut_pos]
        masked_content = truncated_doc[cut_pos:cut_pos+masked_len]
        suffix = truncated_doc[cut_pos + masked_len:]
