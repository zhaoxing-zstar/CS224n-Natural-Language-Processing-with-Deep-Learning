...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, (hn, cn) = self.encoder(X_pack)
        enc_hiddens, _  = nn.utils.rnn.pad_packed_sequence(enc_hiddens, batch_first=True)  # shape (b, src_len, h*2)
        init_decoder_hidden = self.h_projection(torch.cat((hn[0], hn[1]), -1))  # shape: (2, b, h) -> (b, 2*h)
