...
def naiveSoftmaxLossAndGradient(
    ...
    ### END YOUR CODE
    softmaxTerm = softmax(outsideVectors @ centerWordVec)
    loss = -np.log(softmaxTerm[outsideWordIdx])
    gradCenterVec = - outsideVectors[outsideWordIdx] + np.sum(outsideVectors * softmaxTerm.reshape(softmaxTerm.shape[0],-1), axis = 0)
    gradOutsideVecs = softmaxTerm.reshape(softmaxTerm.shape[0],-1) * centerWordVec
    # special case when w==o
...
def negSamplingLossAndGradient(
    ...
    sigmoidOTerm = sigmoid(outsideVectors @ centerWordVec)[outsideWordIdx]
    sigmoidKTerm = sigmoid(- outsideVectors @ centerWordVec)[negSampleWordIndices]
    loss = -np.log(sigmoidOTerm) - np.sum(np.log(sigmoidKTerm))
    gradCenterVec = (sigmoidOTerm - 1) * outsideVectors[outsideWordIdx] + np.sum((1-sigmoidKTerm.reshape(sigmoidKTerm.shape[0],-1)) * outsideVectors[negSampleWordIndices], axis=0)
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = (sigmoidOTerm - 1) * centerWordVec
    for i, neg in enumerate(negSampleWordIndices):
        gradOutsideVecs[neg] += (1-sigmoidKTerm[i]) * centerWordVec
    ### END YOUR CODE
